...
<header>
  ...
  <div class="chips"><span class="chip"><b>${spec.rows.length}</b> linhas</span><span class="chip">gerado <b>${new Date().toLocaleString('pt-BR')}</b></span><span class="chip">clique no cabeçalho pra ordenar</span></div>
</header>
${chartHtml}
<section class="card">
  <div class="toolbar">
    ...
    <div class="actions">
      ...
      <button class="btn" id="btn-csv">📥 Baixar CSV</button>
      <button class="btn" id="btn-copy">📋 Copiar</button>
      <button class="btn" id="btn-pdf">🖨️ Salvar PDF</button>
    </div>
  </div>
...
export async function fetchStockMovers(direction: 'gainers' | 'losers', count: number): Promise<DataViewSpec> {
  ...
  const verb = direction === 'gainers' ? 'valorizaram' : 'caíram';
  ...
  try {
    ...
    if (stocks.length >= 5) {
      return {
        title: `As ${stocks.length} ações da B3 que mais ${verb} hoje`,
        subtitle: 'Bolsa brasileira (B3) — ordenado pela variação do dia',
        columns: ['#', 'Ticker', 'Empresa', 'Preço (R$)', 'Variação %', 'Volume'],
        ...
        chart: {
          type: 'bar', label: 'Variação %',
          labels: stocks.slice(0, 20).map((s: any) => s.stock),
          values: stocks.slice(0, 20).map((s: any) => s.change ?? 0),
        },
        sourceNote: `Fonte: BRAPI (dados da B3) — ${new Date().toLocaleString('pt-BR')}. Volume mínimo de 50K aplicado pra excluir papéis sem liquidez.`,
      };
    }
  } catch { /* cai pro Yahoo */ }
  ...
  if (quotes.length === 0) throw new Error('Nenhuma fonte de cotações respondeu (BRAPI e Yahoo).');
  return {
    title: `As ${quotes.length} ações (EUA) que mais ${verb} hoje`,
    subtitle: 'Mercado americano — screener do Yahoo Finance',
    columns: ['#', 'Ticker', 'Empresa', 'Preço (US$)', 'Variação %', 'Volume'],
