...
- compare_prices: { "action": "compare_prices", "query": "..." } — for ANY price/shopping request ("preço de X", "X mais barato", "quanto custa Y", "onde comprar Z"): scrapes Google Shopping (which aggregates Mercado Livre, Amazon, Magalu, KaBuM…) and opens a price-sorted comparison table. Use THIS as the single first action — do NOT browse store sites or click results one by one.
- google_news: { "action": "google_news", "query": "..." } — for news requests ("notícias de X", "últimas sobre Y", "o que está acontecendo com Z"): scrapes Google News and opens a clickable headline panel (headline, source, when). Use THIS as the single first action — do NOT open news sites one by one.
