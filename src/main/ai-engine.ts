...
- google_news: { "action": "google_news", "query": "..." } — for news requests ("notícias de X", "últimas sobre Y", "o que está acontecendo com Z"): scrapes Google News and opens a clickable headline panel (headline, source, when). When a Tavily API key is configured, structured news results from Tavily are fetched in parallel and merged with the Google scrape for richer coverage. Use this for news by TOPIC with NO specific site. If the user gives a specific news site or URL, do NOT use this — navigate there and use that site's own search/sections instead.
